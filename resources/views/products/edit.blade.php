...
<div class="mb-3 row">
    ...
    <div class="col-md-6">
        @if($product->image)
            <img src="{{ asset('storage/' . $product->image) }}" alt="Current Product Image" class="img-fluid rounded mb-3" style="max-height: 200px;">
        @endif
        <input type="file" class="form-control @error('image') is-invalid @enderror" id="image" name="image" accept="image/*">
        <small class="form-text text-muted">Only image file formats (jpg, jpeg, png, gif) are supported.</small>
        @error('image')
            <span class="text-danger">{{ $message }}</span>
        @enderror
    </div>
